...
# 第五轮：开市时段全链路实测与性能优化（2026-09-14 10:09–10:21）

## 18. 实测环境

- 时间：2026-09-14（周一）10:09–10:21 Asia/Shanghai
- `market/clock` → `phase=trading`，`is_trading=true`
- 服务：`127.0.0.1:8765`（最新代码重启后）

## 19. 首轮结果：28/28 通过

覆盖：clock、4 只行情、4 组搜索、日 K、分时 1m/5m、经典信号、综合评分、持仓、基本面、参考清单、监控状态、手动扫描、提醒流、**价格线触发**、静态资源。

开市实时数据样例：
- 159516 分时最后 10:03/10:08，均价在算
- 日 K 已含 2026-09-14 当日 bar
- 手动扫描产出偏空/偏多/强卖信号
- 设 stop 高于现价 → 扫描立刻出现【跌破止损线】

## 20. 发现的性能问题与修复

| 问题 | 原耗时 | 修复 | 修复后 |
|---|---|---|---|
| 单票取价串行 sina→腾讯 | ~650ms | 两路并行 ThreadPool | **~300–330ms** |
| 基本面每次拉东财全量表 | **~12s** | 断路器打开时直接跳过；有 PE/PB 不再拉表 | 冷 ~1s / 热 **~300ms** |
| 个股行业信息每次打东财 | 2–6s | `_stock_info_cached` 1h 缓存 + 断路器 | 失败不拖慢主路径 |
| 前端提醒轮询固定 20s | 20s | **开市 10s / 休市 30s**；价格线/strong 也推送 | — |

## 21. 修复后复测（同一时段）

```
quote 320ms | portfolio 478ms | signals/pro 481ms
history 423ms | intraday 520ms | scan 624ms
fundamentals 985ms 冷 → 311ms 热
price_break 实测仍可触发
```

## 22. 遗留观察

- 东财仍限流（RemoteDisconnected）；断路器按预期工作
- 日线 120 与 250 的最后一根日期偶发不一致（不同源），已记入 todo
- 行业字段在东财不可用时暂缺（PE/PB 仍有腾讯数据）

## 23. 建议用户操作

1. 浏览器刷新页面，打开「实时提醒」→ 授权系统通知，挂机到收盘
2. 对持仓点「设线」设止损/目标，验证推送
3. 仍需轮换 GitHub PAT

---

# 第六轮：严格多周期信号 + 回测校准 + 今日操作卡（2026-09-14）

## 24. 目标

用户看不懂复杂折线，希望**更精准的操作推荐**，并愿意用 CPU/GPU 换软件能力。选择「信号更严更准 + 本地模型增强」。

## 25. 实现

### 严格信号（`advisor.py`）
- 日线综合分 + **周线结构**投票（周 MA/MACD）
- 只有「分数到阈值 **且** 多周期同向共振」才给 82 置信强建议
- 单周期偏多/偏空：68 置信，措辞更保守
- 分高/分低但未共振：降为观望，避免假信号
- 中间带：结构偏空或分数偏弱 → **回避加仓**（已修一处把偏空误标成“可小仓试错”的逻辑 bug）

### 监控更严
- 买入阈值 60→**65**，强卖 35→**32**
- 推送门槛：置信 **≥68**；强推送需共振（≥80）
- 扫描改走 `advisor.advise_universe`

### 历史回测（`backtest.py` + `/api/backtest`）
- 对单标的近 500 日滚动打分
- 统计不同阈值后 5 日涨跌胜率/平均收益
- 输出「更严阈值」建议（样本不足回落 72/35）
- 159516 实测：≥60 胜率约 55%（n=185），≥72 约 53%（n=70）——说明**不是分越高越准**，故强调共振而非盲目加分

### 今日操作卡（前端新页）
- 建议动作、置信、日周投票、仓位提示、if/then 情景
- 点击卡片可切换当前标的

### 本地模型
- 可选调用 Ollama `qwen2.5:3b` 润色解释
- 本机当前无 Ollama/torch：**自动降级规则白话**，功能完整可用
- 未强行安装大模型，避免占盘/占卡却不能提高预测上限

## 26. 验证

```
advice 159516 score=36 → 回避加仓 / 偏空结构 conf=62
advice 511260 score=66 → 结构偏多中性带
backtest 159516 ok bars=500
scan 走严格路径
frontend 含「今日操作」
```
...
更严规则 = **更少噪音、更可解释**，不等于每次都更准。GPU 本地模型可提升解释质量，不能从免费行情变出高胜率预测。回测是样本内教学工具。
